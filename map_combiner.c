...
int main( int argc, char *argv[]) {
    ...
    while ((nread = getline(&line, &len, stream)) != -1) {
      ...
      if ( remove_year_flag ) {
        sscanf(line, "%s %s %*d %d", new_headword_mixed_case, new_tailword_mixed_case, &new_count);
      } else {
        sscanf(line, "%s %s %d", new_headword_mixed_case, new_tailword_mixed_case, &new_count);
      }
